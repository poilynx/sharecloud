...
final class File extends ModelBase {
	...
	protected function assign(array $row) {
		...
		/*
		if((!System::getUser()->isAdmin) && (System::getUser() == NULL || System::getUser()->uid != $this->uid)) {
			$this->readonly = true;	
		}
		 */
		if(System::getUser() != NULL) {
			if(!System::getUser()->isAdmin && System::getUser()->uid != $this->uid) {
	...
	public function save() {
		...
		if($this->isNewRecord) {			
			...
			$sql = System::getDatabase()->prepare('SELECT _id FROM files WHERE folder_ID = :folderid AND filename = :filename');
			$sql->execute(array(':folderid'=>$this->folderid,':filename'=>$this->filename));
			if($sql->rowCount() > 0) {
	...
	public function delete() {
		...
		if(System::isAdmin == false) {
			$sql = System::getDatabase()->prepare('SELECT _id FROM files WHERE _id = :id AND user_ID = :uid');
			$sql->execute(array(':id' => $this->id, ':uid' => System::getUser()->uid));
			if($sql->rowCount() == 0) {
				die("{'success':false,'message':'sss'}");
			}
		}
		*/
		$success = @unlink($this->getAbsPath());
		
		#die("{'success':false,'message':'sss1'}");
		if($success == true) {		
			$sql = System::getDatabase()->prepare('DELETE FROM files WHERE _id = :id');
			$sql->execute(array(':id' => $this->id));
		} else {
	...
	public static function search($keywords) {
		$result = NULL;//_id list that be found.
		$query = 'select _id from files where filename like :keyword;';
		$db = System::getDatabase();
		$sql = $db->prepare($query);
		if(!is_array($keywords)) {
			die('`keywords` not array.');
		}
		//echo "<pre>";
		foreach($keywords as $keyword) {
			...
			$result = $result===NULL? $res : array_intersect($result,$res);
		}
		$result_t = array();
		...
		$query = "select * from files where _id in (?";
